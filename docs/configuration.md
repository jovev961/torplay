...
When a ready provider item is associated with a TMDB movie or show, Play checks it before searching torrent sources. Season-pack files with clear episode names map automatically; a unique match to a TMDB episode title can also map a file. Debrid Library still lets you correct a show episode mapping manually. To use an item added outside TorPlay from a title page, open it in Debrid Library and explicitly associate it with that TMDB title and, for a show, its season. Ambiguous files remain unmapped. TorPlay checks the provider's current resource and creates a fresh media link before playback; a stale or unavailable item falls back to normal source search.
