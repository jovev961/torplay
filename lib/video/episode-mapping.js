...
export function resolveEpisodeFile(infoHash, context, files, season = context?.season,
  ...
  return mappedEpisodeFile(infoHash, context, files, season, episode, options.database, "manual")
    || findEpisodeFile(files, Number(season), Number(episode), options)
    || mappedEpisodeFile(infoHash, context, files, season, episode, options.database);
}
