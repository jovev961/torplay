...
function millisecondsToTimestamp(value) {
  ...
  return hours > 0
    ? `${String(hours).padStart(2, "0")}:${minutes}:${seconds}.${milliseconds}`
    : `${minutes}:${seconds}.${milliseconds}`;
}

export function shiftWebVtt(input, offsetMs = 0) {
  if (!Number.isFinite(offsetMs) || offsetMs === 0) return input;
  return input.replace(
    /((?:\d{1,2}:)?\d{2}:\d{2}\.\d{3})\s*-->\s*((?:\d{1,2}:)?\d{2}:\d{2}\.\d{3})([^\n]*)/g,
    (_line, start, end, settings) => {
      const shiftedStart = timestampToMilliseconds(start) + offsetMs;
      const shiftedEnd = timestampToMilliseconds(end) + offsetMs;
      if (shiftedEnd <= 0) return "00:00.000 --> 00:00.001";
      return `${millisecondsToTimestamp(shiftedStart)} --> ${millisecondsToTimestamp(shiftedEnd)}${settings}`;
    },
  );
}

export function subtitleToWebVtt(input, format, options = {}) {
