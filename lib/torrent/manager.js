...
const TRANSCODE_VIDEO_TYPES = new Map([
  [".3g2", "video/3gpp2"], [".3gp", "video/3gpp"], [".avi", "video/x-msvideo"],
  [".divx", "video/x-msvideo"], [".flv", "video/x-flv"], [".m2ts", "video/mp2t"],
  [".mkv", "video/x-matroska"], [".mov", "video/quicktime"], [".mpg", "video/mpeg"],
  [".mpeg", "video/mpeg"], [".mts", "video/mp2t"], [".ogm", "video/ogg"],
  [".ogv", "video/ogg"], [".ts", "video/mp2t"], [".vob", "video/mpeg"],
  [".wmv", "video/x-ms-wmv"],
]);
...
async function fetchTorrentFile(rawUrl, timeoutMs, signal) {
  let url;
  try {
    url = new URL(rawUrl);
  } catch {
...
async function resolveProviderSource(source, signal) {
  if (typeof source?.resolver !== "function") return false;
  const resolved = await source.resolver({ signal });
...
export function classifyVideoFile(name) {
  ...
  if (nativeMimeType) return { mimeType: nativeMimeType, sourceMimeType: nativeMimeType, playbackMode: "native" };
  const sourceMimeType = TRANSCODE_VIDEO_TYPES.get(extension);
  if (sourceMimeType) {
    return { mimeType: "video/mp4", sourceMimeType, playbackMode: "transcode" };
  }
  return null;
}
...
export async function inspectTorrentSource(source, context = {}, { signal } = {}) {
  signal?.throwIfAborted();
  if (Buffer.isBuffer(source?.torrentInput)) {
  ...
  if (attemptedDownloadUrl) {
    try {
      const input = await fetchTorrentFile(attemptedDownloadUrl, TORRENT_VALIDATION_FETCH_TIMEOUT_MS, signal);
      ...
      await resolveProviderSource(source, signal);
  ...
  const input = await fetchTorrentFile(source.downloadUrl, TORRENT_VALIDATION_FETCH_TIMEOUT_MS, signal);
  const inspection = await inspectTorrentInput(input, context);
...
function releasePrefetchLease(session) {
  if (session.prefetch?.range) {
    updatePieceReferences(session.resource, session.prefetch.range, -1);
  }
}

function prefixDownloadedBytes(session, file, range, targetBytes) {
  const torrent = session.resource.torrent;
  if (!range || !torrent.bitfield?.get) return 0;
  const fileStart = file.offset;
  const targetEnd = file.offset + targetBytes;
  let downloaded = 0;
  for (let piece = range.start; piece <= range.end; piece += 1) {
    if (!torrent.bitfield.get(piece)) continue;
    const pieceStart = piece * torrent.pieceLength;
    const pieceEnd = Math.min(pieceStart + torrent.pieceLength, fileStart + file.length);
    downloaded += Math.max(0, Math.min(pieceEnd, targetEnd) - Math.max(pieceStart, fileStart));
  }
  return Math.min(targetBytes, downloaded);
}

function prefetchSnapshot(session) {
  const prefetch = session.prefetch;
  if (!prefetch) return { state: "idle" };
  const file = session.resource.torrent.files[Number(prefetch.fileId)];
  if (!file) return { state: "failed", fileId: prefetch.fileId,
    targetBytes: prefetch.targetBytes, downloadedBytes: 0 };
  const downloadedBytes = prefixDownloadedBytes(session, file, prefetch.range, prefetch.targetBytes);
  return {
    state: downloadedBytes >= prefetch.targetBytes ? "ready" : "preparing",
    fileId: prefetch.fileId,
    targetBytes: prefetch.targetBytes,
    downloadedBytes,
  };
}

export function prefetchVideoFile(session, file, targetBytes) {
  const fileId = String(session.resource.torrent.files.indexOf(file));
  if (fileId === "-1" || file.length <= 0) return null;
  const boundedTarget = Math.max(1, Math.min(file.length, Math.ceil(Number(targetBytes) || 0)));
  const requested = bytePieceRange(session.resource, file, 0, boundedTarget - 1);
  const current = session.prefetch;
  if (current?.fileId === fileId && current.targetBytes >= boundedTarget) {
    session.lastActivity = Date.now();
    return prefetchSnapshot(session);
  }

  updatePieceReferences(session.resource, requested, 1);
  releasePrefetchLease(session);
  session.prefetch = {
    fileId,
    range: requested,
    targetBytes: boundedTarget,
  };
  session.lastActivity = Date.now();
  return prefetchSnapshot(session);
}

export function getVideoFilePrefetch(session, file = null) {
  session.lastActivity = Date.now();
  if (file && session.prefetch?.fileId !== String(session.resource.torrent.files.indexOf(file))) {
    return { state: "idle" };
  }
  return prefetchSnapshot(session);
}

export function cancelVideoFilePrefetch(session, file = null) {
  if (!session.prefetch) return false;
  if (file && session.prefetch.fileId !== String(session.resource.torrent.files.indexOf(file))) {
    return false;
  }
  releasePrefetchLease(session);
...
export function bufferVideoFile(session, file, byteRange = null) {
  ...
  if (session.prefetch?.fileId === fileId) cancelVideoFilePrefetch(session, file);
  session.lastActivity = Date.now();
}
...
export async function stopTorrent(id) {
  ...
  stopActiveConversion(session);
  releaseSelectedFile(session);
  cancelVideoFilePrefetch(session);
  const resource = session.resource;
