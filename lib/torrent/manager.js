...
export async function resolveTorrentInput(source) {
  if (typeof source?.resolver === "function" && !source.torrentInput && !source.downloadUrl && !source.magnet) {
    const resolved = await source.resolver();
    if (!resolved || typeof resolved !== "object") throw new Error("The provider could not resolve torrent metadata.");
  ...
  if (source?.downloadUrl) {
    try {
      const input = await fetchTorrentFile(source.downloadUrl, TORRENT_START_FETCH_TIMEOUT_MS);
...
export async function inspectTorrentSource(source, context = {}) {
  ...
  if (Buffer.isBuffer(source?.torrentInput)) {
    const parsed = await parseTorrentMetadata(source.torrentInput);
    const videos = (parsed.files ?? []).flatMap((file) => {
      const video = classifyVideoFile(file.name);
      return video ? [{ name: file.name, path: safeTorrentRelativePath(file.path, file.name), size: file.length, ...video }] : [];
    });
    const selected = context.type === "show"
      ? findEpisodeFile(videos, Number(context.season), Number(context.episode))
      : findLargestFile(videos);
    return selected ? { playbackMode: selected.playbackMode } : null;
  }
  if (!source?.downloadUrl) return null;

  const input = await fetchTorrentFile(
    source.downloadUrl,
    TORRENT_VALIDATION_FETCH_TIMEOUT_MS,
  );
  const parsed = await parseTorrentMetadata(input);
  const videos = (parsed.files ?? []).flatMap((file) => {
    const video = classifyVideoFile(file.name);
  ...
  return { playbackMode: selected.playbackMode };
}
...
function toPublicSession(session) {
  ...
  return {
    ...
    files: resource.status === "ready" ? listPublicVideoFiles(session) : [],
...
export function findSessionEpisode(id, season, episode) {
  ...
  const file = findEpisodeFile(listPublicVideoFiles(session), season, episode, {
