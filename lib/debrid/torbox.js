...
export class TorBoxProvider {
  ...
  async findExisting(infoHash) {
    for (let offset = 0; offset < 5_000; offset += 1_000) {
      const list = await this.request(`torrents/mylist?offset=${offset}&limit=1000`);
      ...
      if (list.length < 1_000) break;
    }
    return null;
  }

  async resolveStream(torrent, selection) {
    let resource = await this.findExisting(torrent.infoHash);
    let owned = false;
    try {
      if (!resource) {
      ...
      if (owned && Number.isInteger(resource?.id)) {
        await this.cleanup({ id: resource.id, owned: true }).catch(() => {});
      }
      throw error;
    }
  }
  ...
  async cleanup(resource) {
    ...
    await this.request("torrents/controltorrent", {
      ...
      body: JSON.stringify({ torrent_id: resource.id, operation: "delete", all: false }),
    });
  }
}
