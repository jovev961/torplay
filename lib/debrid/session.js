...
import {
  beginStream, classifyVideoFile, findSessionEpisode, getTorrentMappingTarget, getTorrentMediaContext,
...
import { isRefreshableRemoteFailure, probeRemoteUrl, proxyRemoteFile } from "./stream.js";
import { createDebridJob, rememberReadyResource } from "./library.js";
...
function remoteFile(session, descriptor, index) {
  ...
  return {
    ...
    async arrayBuffer() {
      ...
      }), this, (force, previousUrl) => resolveRemoteUrl(session, this, force, previousUrl));
...
function publicSession(session) {
  const files = session.resource.torrent.files.flatMap((file, index) => {
  ...
  const suggested = session.mediaContext?.type === "show"
    ? resolveEpisodeFile(session.torrent.infoHash, session.mediaContext, files,
      session.mediaContext.season, session.mediaContext.episode,
      { allowSingleFileFallback: false }) : null;
  return {
    ...
    files,
    suggestedFileId: suggested?.id || null,
  };
}
...
export async function startUsenetPlayback(job, normalized, provider, dependencies = {}) {
  ...
  const session = {
    ...
    streamUrls: new Map([[selected.providerId, stream.url]]), streamUrlRefreshes: new Map(),
    resolution: [],
  };
...
function chosenMedia(files, context, infoHash = null) {
  if (context?.type === "show") {
    return resolveEpisodeFile(infoHash, context, files, context.season, context.episode, {
      allowSingleFileFallback: !infoHash,
    });
  }
...
async function resolvePlayableStream(providerId, provider, torrent, selection, availability, probe) {
  try {
    const stream = await provider.resolveStream(torrent, selection, availability);
    await probe(stream.url);
    return stream;
  } catch (error) {
    if (providerId !== "real-debrid" || !isRefreshableRemoteFailure(error)) throw error;
    console.info("Real-Debrid media link rejected; retrying with remote traffic", {
      code: error.code || "unavailable", upstreamStatus: error.upstreamStatus || null,
    });
  }
  const stream = await provider.resolveStream(torrent, selection, availability, { remote: true });
  await probe(stream.url);
  return stream;
}
...
export async function startPlaybackSource(source, dependencies = {}) {
  ...
  if (config.mode === "local" && action !== "ready" && action !== "remote") {
    return (dependencies.startLocal || startTorrent)(source);
  }
  if (config.mode === "local") {
    throw new DebridError("debrid-disabled", "Debrid playback is disabled in Settings.", 422);
  }
  if (action === "local") {
  ...
  const availableProviders = config.priority.filter((id) => config.credentials[id]
    && (!["ready", "remote"].includes(action) || id === dependencies.remoteProvider));
  if (!availableProviders.length && action === "resolve" && config.mode === "prefer-debrid" && localAllowed) {
    return (dependencies.startLocal || startTorrent)(source);
  }
  ...
  try {
    ...
    if (!localAllowed || action !== "resolve") throw error;
    return (dependencies.startLocal || startTorrent)(source);
  }
  const resolution = [];
  for (const id of availableProviders) {
    ...
    try {
      ...
      const selected = chosenMedia(candidateFiles(availability.files), source.mediaContext, descriptor.infoHash);
      if (!selected) {
        resolution.push({ provider: id, status: "file-not-found" });
        logResolution(id, "available", Date.now() - started, "file-not-found");
        continue;
      }
      const stream = await resolvePlayableStream(id, provider, descriptor, selected, availability,
        dependencies.probe || probeRemoteUrl);
      resource = stream.resource;
      if (resource?.id) {
        await rememberReadyResource(id, provider, descriptor.infoHash, resource.id, source,
          { ...dependencies, resourceOwned: resource.owned })
  ...
  if (action === "ready") {
    throw new DebridError("not-ready", "This torrent is no longer ready on the chosen provider. Choose another option.", 409);
  }
  ...
  if (action === "remote" || config.unavailableAction === "remote") {
    ...
    if (action !== "remote" && providerId === "torbox" && source.mediaContext?.type === "show") {
      return { kind: "choice", localAllowed, providers: selectableProviders,
        resolution, seasonPack: true };
    }
    if (action === "remote" && providerId === "torbox" && source.mediaContext?.type === "show"
      && dependencies.confirmWholePack !== true) {
      throw new DebridError("confirmation-required", "Confirm that TorBox may download the whole pack.", 422);
    }
...
export async function startDebridResourcePlayback(input, dependencies = {}) {
  ...
  const torrent = { id: resourceId, infoHash: input.infoHash || null };
  const stream = await resolvePlayableStream(providerId, provider, torrent, selection, availability,
    dependencies.probe || probeRemoteUrl);
  const session = {
    id: randomUUID(), backend: "debrid", sourceType: "torrent", providerId, provider,
    torrent, availability, resourceRef: null,
    mediaContext: input.mediaContext || null, releaseName: input.releaseName || selection.name,
    resource: { status: "ready", torrent: { files: [] }, mediaProbes: new Map() },
    selectedFileId: null, activeConversion: null, subtitleDiscoveries: new Map(),
    lastActivity: Date.now(), streams: 0, activeControllers: new Set(),
    streamUrls: new Map([[selection.providerId, stream.url]]), streamUrlRefreshes: new Map(),
    resolution: [],
  };
  session.resource.torrent.files = availability.files.map((file, index) => remoteFile(session, file, index));
  state().sessions.set(session.id, session);
...
export async function resolveRemoteUrl(session, file, force = false, previousUrl = null) {
  const current = session.streamUrls.get(file.providerId);
  if (!force && current) return current;
  if (force && current && previousUrl && current !== previousUrl) return current;
  session.streamUrlRefreshes ||= new Map();
  const pending = session.streamUrlRefreshes.get(file.providerId);
  ...
  const refresh = (async () => {
    const stream = await session.provider.resolveStream(session.torrent, {
      ...
      remote: force && session.providerId === "real-debrid",
    });
    if (force) console.info("Debrid media URL refreshed", {
      ...
      fileId: String(file.providerId), remoteTraffic: session.providerId === "real-debrid",
    });
    session.streamUrls.set(file.providerId, stream.url);
    return stream.url;
  })();
  session.streamUrlRefreshes.set(file.providerId, refresh);
  try { return await refresh; }
  finally {
    if (session.streamUrlRefreshes.get(file.providerId) === refresh) {
      session.streamUrlRefreshes.delete(file.providerId);
    }
  }
}
...
export function findPlaybackSessionEpisode(id, season, episode) {
  ...
  const file = resolveEpisodeFile(session.torrent.infoHash, session.mediaContext,
    publicSession(session).files, season, episode, {
    allowSingleFileFallback: false,
  });
  return file ? { session: publicSession(session), file } : null;
}

export function getPlaybackMappingTarget(id) {
  const session = getDebridSession(id);
  if (!session) return getTorrentMappingTarget(id);
  return { infoHash: session.torrent.infoHash, mediaContext: session.mediaContext,
    files: publicSession(session).files };
}
    ...
    current.server = http.createServer(async (incoming, outgoing) => {
      ...
      try {
        ...
        const response = await proxyRemoteFile(request, match.file,
          (force, previousUrl) => resolveRemoteUrl(match.session, match.file, force, previousUrl));
