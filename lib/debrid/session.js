...
export function makeDebridProvider(id, credentials, dependencies = {}) {
  if (id === "real-debrid") return new RealDebridProvider(credentials, {
    ...dependencies,
    onRefresh: async (next) => updateDebridConfig((config) => ({
      ...config, credentials: { ...config.credentials, "real-debrid": next },
    })),
  });
...
export async function startPlaybackSource(source, dependencies = {}) {
  ...
  if (!availableProviders.length && config.mode === "prefer-debrid" && config.localFallback) {
  ...
  try {
    ...
    if (config.mode === "debrid-only" || !config.localFallback) throw error;
    return (dependencies.startLocal || startTorrent)(source);
  }
  const resolution = [];
  for (const id of availableProviders) {
    ...
    try {
      ...
      if (resource?.owned) await provider.cleanup(resource).catch(() => {});
  ...
  if (config.mode === "debrid-only" || !config.localFallback) {
    const statuses = new Set(resolution.map((item) => item.status));
    const code = statuses.has("file-not-found") ? "file-not-found"
      : [...statuses].some((item) => !["not-cached"].includes(item)) ? "unavailable" : "not-cached";
    const message = code === "file-not-found"
      ? "The requested media file was not found in a ready debrid torrent."
      : code === "unavailable" ? "The configured debrid providers are unavailable."
        : "No configured debrid provider has the requested file ready.";
    throw new DebridError(code, message, 422);
  }
...
export async function stopPlayback(id) {
  ...
  if (resource?.id) {
    ...
    if (registered && --registered.refs <= 0) {
      ...
      if (registered.owned) await registered.provider.cleanup(registered).catch(() => {});
    }
  }
