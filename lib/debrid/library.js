...
import { autoMapEpisodeFiles, episodeMappingsForFiles, replaceEpisodeFileMappings, resolveEpisodeFile, saveEpisodeFileMapping } from "../video/episode-mapping.js";
...
const readyReadStates = new WeakMap();
...
function readyReadState(db) {
  let state = readyReadStates.get(db);
  if (!state) {
    state = { cache: new Map(), pending: new Map(), cooldown: new Map() };
    readyReadStates.set(db, state);
  }
  return state;
}
...
function retryDelay(value) {
  ...
  return Number.isFinite(date) ? Math.min(Math.max(date - stamp(), 0), 5 * 60_000) : 30_000;
}

async function readReadyResource(id, connected, resourceId, db) {
  const state = readyReadState(db);
  const key = `${id}:${connected.accountHash}:${resourceId}`;
  const cached = state.cache.get(key);
  if (cached?.until > stamp()) return { raw: cached.raw, fetched: false };
  if ((state.cooldown.get(id) || 0) > stamp()) {
    throw new DebridError("rate-limited", "The debrid provider is temporarily rate limited.", 429);
  }
  let pending = state.pending.get(key);
  if (!pending) {
    pending = connected.provider.getResource(resourceId);
    state.pending.set(key, pending);
  }
  try {
    const raw = await pending;
    state.cache.set(key, { raw, until: stamp() + 60_000 });
    if (state.cache.size > 200) state.cache.delete(state.cache.keys().next().value);
    return { raw, fetched: true };
  } catch (error) {
    state.cache.delete(key);
    if (error.code === "rate-limited" || error.upstreamStatus === 429) {
      state.cooldown.set(id, stamp() + retryDelay(error.retryAfter));
    }
    throw error;
  } finally {
...
export async function mapDebridEpisodeFile(id, resourceId, fileId, season, episode, dependencies = {},
  ...
  const target = item.episodeMappings.find((entry) => entry.season === episodeContext.season
    && entry.episode === episodeContext.episode);
  if (target && target.fileId !== file.providerId
    && String(options.expectedTargetFileId || "") !== target.fileId) {
    invalid("This episode is already mapped to another file. Refresh and confirm the replacement.", 409);
  }
  if (!target && options.expectedTargetFileId) {
    invalid("This episode mapping changed. Refresh the file list and try again.", 409);
  }
  try {
    if (options.replaceExisting) {
      replaceEpisodeFileMappings(link?.info_hash || row.info_hash, episodeContext, file,
        item.episodeMappings.filter((entry) => entry.fileId === file.providerId), db);
    } else {
      saveEpisodeFileMapping(link?.info_hash || row.info_hash, episodeContext, file, db);
    }
  }
...
export async function getDebridItem(id, resourceId, dependencies = {}) {
  ...
  if (!dependencies.skipEpisodeMapping && context?.type === "show" && Array.isArray(raw.files)) {
...
export async function listReadyDebridEpisodes(tmdbId, dependencies = {}) {
  const showId = Number(tmdbId);
  if (!Number.isSafeInteger(showId) || showId < 1) invalid("Invalid show identity.");
  const config = dependencies.config || await readDebridConfig();
  if (config.mode === "local") return { episodes: [], temporarilyUnavailable: false };
  const db = dependencies.database || getDatabase();
  const links = db.prepare(`SELECT * FROM debrid_media_links
    WHERE media_type = 'show' AND tmdb_id = ? ORDER BY updated_at DESC`).all(showId);
  const episodes = new Map();
  let temporarilyUnavailable = false;
  for (const id of config.priority) {
    if (!config.credentials[id]) continue;
    const providerLinks = links.filter((link) => link.provider === id);
    if (!providerLinks.length) continue;
    if ((readyReadState(db).cooldown.get(id) || 0) > stamp()) {
    ...
    try { connected = await connectedProvider(id, dependencies); }
      ...
      if (error.code === "rate-limited" || error.upstreamStatus === 429) {
        readyReadState(db).cooldown.set(id, stamp() + retryDelay(error.retryAfter));
      }
      temporarilyUnavailable = true;
      console.info("Ready debrid account unavailable", { provider: id, code: error.code || "unavailable" });
      continue;
    }
    for (const link of providerLinks.filter((entry) => entry.account_hash === connected.accountHash)) {
      try {
        const { raw, fetched } = await readReadyResource(id, connected, link.resource_id, db);
        if (String(raw.hash || "").toLowerCase() !== link.info_hash) {
          db.prepare(`DELETE FROM debrid_media_links WHERE provider = ? AND account_hash = ?
            AND resource_id = ?`).run(id, connected.accountHash, link.resource_id);
          readyReadState(db).cache.delete(`${id}:${connected.accountHash}:${link.resource_id}`);
          continue;
        }
        const item = await getDebridItem(id, link.resource_id, {
          ...dependencies, connectedProvider: connected, raw,
          skipEpisodeMapping: !fetched,
        });
        if (item?.status !== "ready") continue;
        const selected = new Set(item.files.filter((file) => file.selected).map((file) => file.providerId));
        for (const mapping of item.episodeMappings) {
          if (!selected.has(mapping.fileId)) continue;
          const key = `${mapping.season}:${mapping.episode}`;
          if (!episodes.has(key)) episodes.set(key, {
            season: mapping.season, episode: mapping.episode, provider: id,
          });
        }
      } catch (error) {
        if (error.upstreamStatus === 404) {
          db.prepare(`DELETE FROM debrid_media_links WHERE provider = ? AND account_hash = ?
            AND resource_id = ?`).run(id, connected.accountHash, link.resource_id);
          readyReadState(db).cache.delete(`${id}:${connected.accountHash}:${link.resource_id}`);
        } else temporarilyUnavailable = true;
        console.info("Ready debrid resource unavailable", { provider: id, code: error.code || "unavailable" });
      }
    }
  }
  return { episodes: [...episodes.values()].sort((a, b) => a.season - b.season || a.episode - b.episode),
    temporarilyUnavailable };
}

async function directDebridCandidates(context, dependencies = {}) {
  ...
  const config = dependencies.config || await readDebridConfig();
  if (config.mode === "local") return { candidates: [], temporarilyUnavailable: false };
  const db = dependencies.database || getDatabase();
  const candidates = [];
  let temporarilyUnavailable = false;
  for (const id of config.priority) {
    if (dependencies.selection && id !== dependencies.selection.provider) continue;
    if (!config.credentials[id]) continue;
    ...
    if ((readyReadState(db).cooldown.get(id) || 0) > stamp()) continue;
    let connected;
    try { connected = await connectedProvider(id, dependencies); }
    catch (error) {
      if (error.code === "rate-limited" || error.upstreamStatus === 429) {
    ...
    for (const link of links.filter((entry) => entry.account_hash === connected.accountHash)) {
      if (dependencies.selection && String(link.resource_id) !== String(dependencies.selection.resourceId)) continue;
      try {
        const { raw } = await readReadyResource(id, connected, link.resource_id, db);
        if (String(raw.hash || "").toLowerCase() !== link.info_hash) {
          db.prepare(`DELETE FROM debrid_media_links WHERE provider = ? AND account_hash = ?
            AND resource_id = ?`).run(id, connected.accountHash, link.resource_id);
          continue;
        ...
        const item = await getDebridItem(id, link.resource_id,
          { ...dependencies, connectedProvider: connected, raw,
            skipEpisodeMetadata: true, skipEpisodeMapping: true });
        if (item?.status !== "ready") continue;
        ...
        candidates.push({ provider: id, resourceId: String(link.resource_id), name: item.name,
          fileId: String(fileId), providerFileId: file.providerId, connected, raw, item,
          requestedMediaContext });
        if (!dependencies.collectAll) return { candidates, temporarilyUnavailable };
      } catch (error) {
        if (error.code === "rate-limited" || error.upstreamStatus === 429) {
          readyReadState(db).cooldown.set(id, stamp() + retryDelay(error.retryAfter));
        }
        ...
        if (error.upstreamStatus !== 404) temporarilyUnavailable = true;
        console.info("Direct debrid resource unavailable", { provider: id, code: error.code || "unavailable" });
      }
    }
  }
  return { candidates, temporarilyUnavailable };
}

export async function inspectDirectDebridAvailability(context, dependencies = {}) {
  const { candidates, temporarilyUnavailable } = await directDebridCandidates(context,
    { ...dependencies, collectAll: true });
  return { sources: candidates.map(({ provider, resourceId, name }) => ({ provider, resourceId, name })),
    temporarilyUnavailable };
}

export async function resolveDirectDebridPlayback(context, dependencies = {}) {
  const { candidates } = await directDebridCandidates(context, dependencies);
  const selection = dependencies.selection;
  const candidate = candidates.find((entry) => !selection || (entry.provider === selection.provider
    && entry.resourceId === String(selection.resourceId)));
  if (!candidate) return { kind: "miss" };
  const session = await playDebridItem(candidate.provider, candidate.resourceId, candidate.providerFileId,
    { ...dependencies, connectedProvider: candidate.connected, raw: candidate.raw,
      item: candidate.item, requestedMediaContext: candidate.requestedMediaContext });
  return { kind: "hit", session, fileId: candidate.fileId };
}
