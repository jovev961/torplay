...
import { episodeMappingsForFiles, resolveEpisodeFile, saveEpisodeFileMapping } from "../video/episode-mapping.js";
...
function normalize(id, raw, row = null, db = getDatabase()) {
  ...
  return {
    ...
    episodeMappings: episodeMappingsForFiles(infoHash, context, videoFiles),
  ...
  const results = await Promise.all(selected.map(async (id) => {
    ...
    try {
      ...
      const items = await Promise.all(list.map(async (raw) => {
        ...
        return normalize(id, raw, row, db);
      }));
...
export async function mapDebridEpisodeFile(id, resourceId, fileId, season, episode, dependencies = {}) {
  ...
  if (!row) invalid("This resource has no show context in TorPlay.", 404);
  const context = row.media_context_json ? JSON.parse(row.media_context_json) : null;
  ...
  try { saveEpisodeFileMapping(row.info_hash, episodeContext, file, db); }
...
export async function getDebridItem(id, resourceId, dependencies = {}) {
  ...
  const item = normalize(id, raw, row, db);
