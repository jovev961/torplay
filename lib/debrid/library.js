...
import { findEpisodeFile, findLargestFile, matchesEpisode, safeTorrentRelativePath } from "../video/episode.js";
...
function normalize(id, raw, row = null) {
  ...
  return {
    ...
    status: ready ? "ready" : failed ? "failed" : processing ? "processing"
      : /queue|pause|stall/i.test(String(raw.status || raw.download_state || "")) ? "queued" : "downloading",
    progress, files: files.filter((file) => classifyVideoFile(file.name))
    ...
    selectedFileId: row?.selected_file_id || null,
...
export async function listDebridLibrary({ provider: filter = "all", page = 1, limit = 50, fresh = false } = {}, dependencies = {}) {
  ...
  const results = await Promise.all(selected.map(async (id) => {
    ...
    try {
      ...
      const items = await Promise.all(list.map(async (raw) => {
...
function chosen(files, context) {
  const candidates = files.filter((file) => classifyVideoFile(file.name) && !/sample/i.test(file.name));
  if (context?.type === "show") return findEpisodeFile(candidates.map((file) => ({ ...file, relativePath: file.path })),
    Number(context.season), Number(context.episode), { allowSingleFileFallback: true });
...
async function selectRealDebrid(provider, raw, row, db) {
  ...
  const selected = chosen(files, context);
  if (!selected) invalid("The requested movie or episode was not found in this torrent.", 422);
  const episodeFiles = row.selection_scope === "all" && context?.type === "show"
    ? files.filter((file) => classifyVideoFile(file.name)
      && !/sample/i.test(file.name) && /S\d{1,2}E\d{1,3}/i.test(file.name)) : [selected];
  await provider.selectFiles(raw.id, [...new Set(episodeFiles.map((file) => file.providerId))]);
  db.prepare(`UPDATE debrid_resources SET selected_file_id = ?, updated_at = ?
    ...
    .run(selected.providerId, stamp(), row.account_hash, String(raw.id), row.selection_scope);
}
...
export async function getDebridItem(id, resourceId, dependencies = {}) {
  ...
  let raw;
  try { raw = await provider.getResource(resourceId); }
  ...
  const item = normalize(id, raw, row);
  ...
  if (row && !row.selected_file_id && item.files.length) {
    const context = row.media_context_json ? JSON.parse(row.media_context_json) : null;
    const selected = chosen(item.files.filter((file) => file.selected), context);
    if (selected) {
      db.prepare(`UPDATE debrid_resources SET selected_file_id = ?, updated_at = ?
...
export async function rememberCreatedResource(id, provider, infoHash, resourceId, source, dependencies = {}) {
  const account = await provider.getAccountInfo();
  ...
  db.prepare(`INSERT OR IGNORE INTO debrid_resources
    ...
    VALUES (?, ?, ?, ?, ?, ?, 'all', 'torplay', ?, ?)`).run(id, accountHash, infoHash,
    String(resourceId), String(source.releaseName || "Torrent").slice(0, 300),
    source.mediaContext ? JSON.stringify(source.mediaContext) : null, now, now);
}
...
export async function createDebridJob(id, descriptor, { scope = "episode" } = {}, dependencies = {}) {
  ...
  const scopeKey = scope === "all" ? "all" : context?.type === "show"
    ? `episode:${context.season}:${context.episode}` : "movie";
  ...
  try {
    ...
    if (found) {
      const item = await getDebridItem(id, found.id, dependencies);
      if (!item) throw new DebridError("unavailable", "The matching provider resource disappeared. Try again shortly.", 503);
      const match = item && chosen(item.files, context);
      const prior = rowFor(db, id, accountHash, found.id);
      const selectionCompatible = !prior || prior.selection_scope === scopeKey || prior.selection_scope === "all";
      if (id === "torbox" || (found.status === "waiting_files_selection" && selectionCompatible)
        || (!item.files.length && found.status !== "downloaded" && selectionCompatible)
        || (match && item.files.some((file) => file.providerId === match.providerId && file.selected))) {
        const now = stamp();
...
export async function playDebridItem(id, resourceId, fileId, dependencies = {}) {
  ...
  const { provider } = await connectedProvider(id, dependencies);
  ...
  let mediaContext = item.mediaContext;
  if (mediaContext?.type === "show" && item.selectedFileId !== file.providerId) {
    const match = /S(\d{1,2})E(\d{1,3})/i.exec(file.path || file.name);
    mediaContext = match && matchesEpisode(file.path || file.name, Number(match[1]), Number(match[2]))
      ? { ...mediaContext, season: Number(match[1]), episode: Number(match[2]) } : null;
  } else if (mediaContext?.type === "movie" && item.selectedFileId !== file.providerId) {
    mediaContext = null;
  }
  return startDebridResourcePlayback({ providerId: id, provider, resourceId, availability,
