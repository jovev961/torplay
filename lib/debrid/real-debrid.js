...
export class RealDebridProvider {
  ...
  async checkAvailability(torrent) {
    return (await this.checkAvailabilityMany([torrent.infoHash]))[torrent.infoHash] || { status: "miss" };
  }

  async checkAvailabilityMany(infoHashes) {
    ...
    const wanted = new Set(infoHashes.map((hash) => String(hash).toLowerCase()));
    ...
    for (let page = 1; page <= 100; page += 1) {
      ...
      for (const item of list) {
        ...
        if (wanted.has(hash) && item.status === "downloaded" && !matches.has(hash)) {
          matches.set(hash, item);
        }
      }
      if (matches.size === wanted.size || list.length < 500) break;
      if (page === 100) {
        throw new DebridError("account-scan-incomplete", "Could not finish checking Real-Debrid account torrents.", 503);
      }
    }
    const result = Object.fromEntries([...wanted].map((hash) => [hash, { status: "miss" }]));
    for (const [hash, match] of matches) {
  ...
  async resolveStream(_torrent, selection, availability, { remote = false } = {}) {
    ...
    for (const link of resource.links) {
      const result = await this.request("unrestrict/link", {
        method: "POST", body: form({ link, ...(remote ? { remote: "1" } : {}) }),
      });
