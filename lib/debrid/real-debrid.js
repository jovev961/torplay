...
export class RealDebridProvider {
  ...
  async checkAvailability(torrent) {
    ...
    for (let page = 1; page <= 5; page += 1) {
      ...
      const response = await this.request(page === 1
        ? "torrents?limit=500" : `torrents?limit=500&page=${page}`);
      const list = page > 1 && response && !Array.isArray(response)
        && typeof response === "object" && Object.keys(response).length === 0
        ? [] : requireData(response, Array.isArray);
