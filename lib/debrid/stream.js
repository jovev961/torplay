...
async function publicAddress(hostname, resolveImpl) {
...
export async function openRemoteUrl(raw, {
  ...
  let address;
  try { address = await publicAddress(url.hostname, resolveImpl); }
  catch (error) {
...
export async function probeRemoteUrl(url, options = {}) {
  ...
  response.destroy();
  if (!valid) throw new DebridError("remote-stream-unavailable",
    "The provider stream does not support byte ranges.");
}
...
export async function proxyRemoteFile(request, file, resolveUrl, options = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const url = await resolveUrl(attempt > 0);
    response = await openRemoteUrl(url, {
      range: upstreamRange, signal: request.signal, ...options,
    });
    if (![401, 403, 410].includes(response.statusCode) || attempt > 0) break;
    console.info("Debrid media range URL expired", { status: response.statusCode });
    response.destroy();
  }
  if (response.statusCode !== 206
    ...
    throw new DebridError("remote-stream-unavailable", "The provider returned an invalid media range.");
  }
