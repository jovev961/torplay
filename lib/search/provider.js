...
import { jackettProvider } from "./jackett-service.js";
import { readPersistentCache, writePersistentCache } from "../cache/persistent.js";
import { providerIds } from "./provider-config.js";
import { effectiveNativeSources, readNativeSources } from "../settings/native-sources.js";

// The configured adapter is selected here, never by search or playback consumers.
export const PROVIDER_RESULT_CACHE_TTL_MS = 24 * 60 * 60 * 1_000;
export function configuredProviders(environment = process.env) {
  const savedCustom = readCustomProviders(environment);
  const savedNative = readNativeSources(environment);
  const custom = effectiveCustomProviders(environment, savedCustom).map((provider) => (
    customProviderAdapter(provider, { environment }) || (provider.kind === "jackett"
      ? { id: provider.id, name: provider.name, search: (context, options) => customTorznabAdapter(jackettProvider(provider, environment)).search(context, options) }
      : customTorznabAdapter(provider))
  ));
  const native = effectiveNativeSources(environment, savedNative);
  const available = [
    ...native,
    ...custom,
  ];
  ...
  const ids = explicit !== undefined
    ? providerIds(explicit).flatMap((id) => id === "jackett"
      ? savedCustom.filter((item) => item.kind === "jackett").map((item) => item.id)
      : [id])
    : available.map((item) => item.id);
  return ids.filter((id) => !savedCustom.some((item) => item.id === id && !item.enabled)
    && !savedNative.some((item) => item.id === id && !item.enabled)).map((id) => {
...
function safeCacheValue(candidates) {
  if (!candidates.every((candidate) => typeof candidate.infoHash === "string"
    && /^[a-f\d]{40}$|^[a-z2-7]{32}$/i.test(candidate.infoHash)
    && !candidate.source?.downloadUrl
    && !candidate.source?.resolver)) return null;
...
function safePreviewValue(candidates) {
  return candidates.filter((candidate) => typeof candidate.infoHash === "string"
    && /^[a-f\d]{40}$|^[a-z2-7]{32}$/i.test(candidate.infoHash)).map((candidate) => ({
    ...candidate,
    source: { magnet: `magnet:?xt=urn:btih:${candidate.infoHash}`, downloadUrl: null },
  }));
}

async function execute(provider, context, timeoutMs, cacheOptions, options) {
  const started = performance.now();
  const cacheKey = providerCacheKey(provider, context);
  if (cacheOptions && !options.refresh) {
    ...
    if (cached.hit && Array.isArray(cached.value)) {
      ...
      if (normalized.length === cached.value.length) {
        const candidates = normalized.map((candidate) => ({ ...candidate, cacheCreatedAt: cached.createdAt }));
        options.onBatch?.({ providerId: provider.id, title: context.title, candidates,
          provisional: false, cacheCreatedAt: cached.createdAt });
        options.onTiming?.({ providerId: provider.id, phase: "cache", elapsedMs: Math.round(performance.now() - started) });
        return candidates;
      }
    }
    const preview = readPersistentCache("provider-preview", cacheKey, cacheOptions);
    if (preview.hit && Array.isArray(preview.value)) {
      const normalized = preview.value.map((candidate) => normalizeCandidate(candidate, provider)).filter(Boolean);
      if (normalized.length === preview.value.length && normalized.length) {
        const candidates = normalized.map((candidate) => ({ ...candidate, cacheCreatedAt: preview.createdAt }));
        options.onBatch?.({ providerId: provider.id, title: context.title, candidates,
          provisional: !options.cacheOnly, cacheCreatedAt: preview.createdAt });
        if (options.cacheOnly) return candidates;
      }
    }
  }
  ...
  const abort = () => {
    ...
    rejectAbort?.(options.signal?.reason || new DOMException("Search cancelled.", "AbortError"));
  };
  options.signal?.addEventListener("abort", abort, { once: true });
  try {
    if (options.signal?.aborted) throw options.signal.reason || new DOMException("Search cancelled.", "AbortError");
    ...
    if (cacheOptions && !cacheValue) {
      const preview = safePreviewValue(candidates);
      if (preview.length) writePersistentCache("provider-preview", cacheKey, preview, {
        ...cacheOptions, ttlMs: PROVIDER_RESULT_CACHE_TTL_MS,
      });
    }
    options.onBatch?.({ providerId: provider.id, title: context.title, candidates,
      provisional: false, cacheCreatedAt: null });
    options.onTiming?.({ providerId: provider.id, phase: "network", elapsedMs: Math.round(performance.now() - started) });
    return candidates;
  } catch (error) {
    if (!options.signal?.aborted) options.onTiming?.({ providerId: provider.id,
      phase: error?.status === 504 ? "timeout" : "failed",
...
export async function searchConfiguredProvider(mediaContext, options = {}) {
  ...
  const settled = await Promise.allSettled(
    providers.map((provider) => execute(provider, mediaContext, timeoutMs, cacheOptions, options)),
  );
  if (options.signal?.aborted) throw options.signal.reason || new DOMException("Search cancelled.", "AbortError");
  const failures = [];
