...
import { knabenProvider } from "./native/knaben.js";
import { ytsProvider } from "./native/yts.js";
import { eztvProvider } from "./native/eztv.js";

// The configured adapter is selected here, never by search or playback consumers.
const providersById = [knabenProvider, ytsProvider, eztvProvider, { id: "jackett", name: "Jackett", search: searchJackett }];
export function configuredProviders(environment = process.env) {
  const explicit = environment.TORPLAY_SEARCH_PROVIDERS;
  const ids = explicit !== undefined ? [...new Set(explicit.split(",").map((id) => id.trim()).filter(Boolean))]
    : ["knaben", "yts", "eztv", ...(environment.JACKETT_API_KEY?.trim() && environment.JACKETT_API_KEY.trim() !== "replace-me"
      && environment.JACKETT_URL?.trim() ? ["jackett"] : [])];
  return ids.map((id) => {
    const provider = providersById.find((item) => item.id === id);
