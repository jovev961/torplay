...
export async function searchJackett(mediaContext) {
  ...
  if (type !== "show") return searchIndexers(query, [parameters], config);
  const seasonParameters = { ...parameters };
  delete seasonParameters.ep;
  return searchIndexers(query, [parameters, seasonParameters], config);
}
