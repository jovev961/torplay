...
function categoryNames(definition) {
  const names = Object.values(definition.caps?.categories || {}).map((value) => String(value).trim());
  for (const item of definition.caps?.categorymappings || []) if (item.cat) names.push(String(item.cat).trim());
...
export function definitionCapabilities(definition) {
  ...
  return { mediaTypes, categories, modes };
}
...
export function settingDescriptors(definition, values = {}) {
  return definitionSettings(definition).map((setting) => {
    ...
    return {
      ...
      required: setting.type === "select" && defaultValue === undefined,
      secret,
...
export function validateDefinitionSettings(definition, values = {}, previous = {}) {
  ...
  for (const [name, field] of fields) {
    ...
    if (["text", "password", "select"].includes(field.type)) {
      ...
      if (!value && field.type === "select" && defaultValue === undefined) throw fail(`${field.label || name} is required.`, 400);
...
export async function importDefinition(definitionUrl, options = {}) {
  ...
  return {
    ...
    definition: {
      ...
      categories: imported.capabilities.categories,
...
function settingsFingerprint(definition, settings) {
  const ordered = definitionSettings(definition)
    .filter((field) => !field.type.startsWith("info"))
    .map((field) => [field.name, settings[field.name]]);
  return createHash("sha256").update(JSON.stringify(ordered)).digest("hex");
}

export function rememberVerificationFailure(id, settings, diagnostic) {
  const imported = importedDefinition(id);
  const token = randomUUID();
  imported.pendingVerification = {
    token,
    fingerprint: settingsFingerprint(imported.definition, settings),
    diagnostic,
  };
  return token;
}

export function confirmedVerificationFailure(id, settings, token) {
  const imported = importedDefinition(id);
  const pending = imported.pendingVerification;
  if (!pending || pending.token !== String(token || "")
    || pending.fingerprint !== settingsFingerprint(imported.definition, settings)) {
    throw fail("Verify this configuration again before adding it as unverified.", 409, "CARDIGANN_VERIFICATION_CONFIRMATION_INVALID");
  }
