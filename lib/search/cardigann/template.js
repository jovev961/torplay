...
function pathValue(path, context, locals) {
  if (path === ".") return Object.hasOwn(locals, ".") ? locals["."] : context;
  if (path.startsWith("$")) {
    const [name, ...parts] = path.slice(1).split(".");
    return parts.reduce((value, key) => value?.[key], locals[name]);
  }
...
function atom(token, context, locals) {
  ...
  if (token.startsWith(".") || token.startsWith("$")) return value ?? "";
...
function renderNodes(nodes, context, locals = {}) {
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    if (/^range\s+/.test(directive)) {
      ...
      const objectEntries = !Array.isArray(collection) && collection && typeof collection === "object";
      const values = Array.isArray(collection) ? collection : objectEntries ? Object.entries(collection) : [];
      values.forEach((value, itemIndex) => {
        const nextLocals = { ...locals };
        if (assignment?.[2]) {
          nextLocals[assignment[1]] = objectEntries ? value[0] : itemIndex;
          nextLocals[assignment[2]] = objectEntries ? value[1] : value;
        }
