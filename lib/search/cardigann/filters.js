...
export const CARDIGANN_FILTERS = new Set([
...
function regexPattern(pattern) {
  // These filters already use JavaScript's case-insensitive flag. Cardigann
  // definitions may also prefix the expression with the .NET-style (?i).
...
export function applyFilters(input, filters = [], { now = new Date(), render = (value) => value } = {}) {
  let value = input ?? "";
  for (const step of filters) {
    const args = argumentsFor(step).map((value) => typeof value === "string" ? render(value) : value);
    const text = String(value ?? "");
    switch (step.name) {
      case "querystring": value = new URL(text, "https://cardigann.invalid").searchParams.get(String(args[0])) || ""; break;
      case "regexp": {
        const match = text.match(new RegExp(regexPattern(args[0]), "i"));
        value = match?.[1] ?? match?.[0] ?? "";
        break;
      }
      case "re_replace": value = text.replace(new RegExp(regexPattern(args[0]), "gi"), String(args[1] ?? "")); break;
