...
export function cardigannAdapter(provider, options = {}) {
  return {
    id: provider.id,
    name: provider.name,
    async search(context, { signal, stopAfterFirstPathWithResults = false } = {}) {
      ...
      for (const [pathIndex, path] of paths.slice(0, 10).entries()) {
        ...
        for (const row of rows) {
          ...
          const resolver = download ? ({ signal: resolutionSignal } = {}) => resolveDownload(
            definition, config, session, base, download, result, resolutionSignal || signal,
          ) : null;
        ...
        if (stopAfterFirstPathWithResults && candidates.length) break;
      }
      return candidates;
    },
  };
...
export async function testCardigannProvider(provider, options = {}) {
  try {
    ...
    const results = await adapter.search({ title: "Sintel", type: provider.capabilities.mediaTypes.includes("Movies") ? "movie" : "show", season: 1, episode: 1 }, { signal: options.signal, stopAfterFirstPathWithResults: true });
    if (!results.length) {
