...
function challengeDetected(response, text) {
  ...
  return server.includes("cloudflare") && /cf-chl-|challenge-platform|attention required|just a moment/.test(sample)
    || /captcha|verify you are human|checking your browser|challenge-platform|cf-chl-/.test(sample);
}
...
export function classifyCardigannVerificationFailure(error) {
  ...
  if (error instanceof SyntaxError) {
    return { code: "CARDIGANN_RESPONSE_PARSE_FAILED", message: "The indexer response could not be parsed.", status: 422 };
  }
  return {
