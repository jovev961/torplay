...
export function filterAndRankResults(results, context = {}) {
  ...
  if (type === "movie") {
    const titles = [context.query, ...(Array.isArray(context.aliases) ? context.aliases : [])];
    filtered = results.filter((result) => titles.some((title) => titleMatches(result.title, title)));
  } else if (type === "show") {
