...
import { effectiveNativeSources } from "./native-sources.js";
...
import { normalizeSignalingBaseUrl } from "../watch-together/config.js";
...
function torrentSourceState(environment) {
  return {
    nativeActive: effectiveNativeSources(environment).length > 0,
...
export async function settingsState({
  ...
  return {
    filename,
    torrentSources: torrentSourceState(environment),
    providers: SETTINGS_PROVIDERS.map((provider) => {
      ...
      const configured = requiredFields.length
        ...
        : provider.fields.some((field) => Boolean(configuredValue(environment[field.environment]) || field.defaultValue));
      return {
        id: provider.id,
...
function validateField(field, value) {
  ...
  if (field.kind === "url") {
    ...
    if (field.environment === "FLARESOLVERR_URL" && (url.search || url.hash || (url.pathname !== "/" && url.pathname !== "/v1"))) {
      throw new SettingsError("FlareSolverr must use its base URL without a query or extra path.");
    }
    if (field.environment === "WATCH_TOGETHER_SIGNAL_URL") {
      try { return normalizeSignalingBaseUrl(normalized); }
      catch (error) { throw new SettingsError(error.message); }
    }
...
async function prepareChanges(changes, options) {
  ...
  for (const { provider, payload } of selected) {
    ...
    for (const [id, rawValue] of Object.entries(values)) {
      ...
      if (!field.required && !field.secret && String(rawValue ?? "").trim() === "") {
        delete updates[field.environment];
        removals.add(field.environment);
        continue;
      }
      updates[field.environment] = validateField(field, rawValue);
      if (field.environment === "FLARESOLVERR_URL") {
        try { await validateFlareSolverrEndpoint(updates[field.environment]); }
        catch { throw new SettingsError("FlareSolverr must be on this computer or a private LAN address."); }
      }
...
export function playbackPreferences(environment = process.env) {
...
export function updatePlaybackPreferences(values, options = {}) {
  return serializeSettingsWrite(async () => {
    ...
    const fileValues = parseSettingsEnvironment(source);
    const updates = {};
    for (const [key, variable] of Object.entries(PLAYBACK_FIELDS)) {
      if (environment[variable] !== undefined && environment[variable] !== fileValues[variable]) {
        throw new SettingsError(`${key} is managed by the host environment.`, 409);
      }
      updates[variable] = String(values[key]);
    }
    await atomicWrite(filename, updateSource(source, updates, new Set()), options.platform);
    Object.assign(environment, updates);
    return playbackPreferences(environment);
  });
}
