...
async function updateNativeProvidersNow(ids, options) {
  ...
  if (!selected.length && !jackettIsConfigured(environment)) {
...
export function updateNativeProviderSettings(ids, options = {}) {
  const operation = writeQueue.then(() => updateNativeProvidersNow(ids, options));
...
export function updateProvidersSettings(changes, options = {}) {
  const operation = writeQueue.then(() => updateProvidersNow(changes, options));
  writeQueue = operation.catch(() => {});
  return operation;
}
