...
import { effectiveCustomProviders } from "./torrent-providers.js";
import { serializeSettingsWrite } from "./write-queue.js";
import {
  configuredNativeProviderIds,
  jackettIsConfigured,
  NATIVE_PROVIDER_IDS,
  NATIVE_PROVIDERS,
  selectedNativeProviderIds,
} from "../search/native/registry.js";
...
function nativeProviderState(environment, external) {
  const selected = selectedNativeProviderIds(environment);
  const configuredProviders = configuredNativeProviderIds(environment);
  const unknown = [...selected, ...configuredProviders].find((id) => !NATIVE_PROVIDER_IDS.includes(id));
  if (unknown) throw new SettingsError(`Unknown native torrent provider: ${unknown}.`, 500);
  if (selected.some((id) => !configuredProviders.includes(id))) {
    throw new SettingsError("Enabled native providers must also be configured.", 500);
  }
  const fullOverride = environment.TORPLAY_SEARCH_PROVIDERS !== undefined;
  return {
    managedExternally: fullOverride || external.has("TORPLAY_NATIVE_PROVIDERS")
      || external.has("TORPLAY_CONFIGURED_NATIVE_PROVIDERS"),
    overrideActive: fullOverride,
    jackettActive: jackettIsConfigured(environment),
    customActive: effectiveCustomProviders(environment).length > 0,
    providers: NATIVE_PROVIDERS.map(({ id, name, description, mediaTypes }) => ({
      id,
      name,
      description,
      mediaTypes,
...
export async function settingsState({
  ...
  return {
    filename,
    torrentSources: nativeProviderState(environment, external),
...
function normalizedNativeProviderIds(ids) {
  if (!Array.isArray(ids)) throw new SettingsError("Enabled native providers must be an array.");
  const unique = [...new Set(ids)];
  if (unique.some((id) => typeof id !== "string" || !NATIVE_PROVIDER_IDS.includes(id))) {
    throw new SettingsError("Native provider selection contains an unknown provider.");
  }
  return NATIVE_PROVIDER_IDS.filter((id) => unique.includes(id));
}

async function updateNativeProvidersNow(ids, options) {
  ...
  if (environment.TORPLAY_SEARCH_PROVIDERS !== undefined) {
    throw new SettingsError("Native providers are controlled by TORPLAY_SEARCH_PROVIDERS.", 409);
  }
  if (external.has("TORPLAY_NATIVE_PROVIDERS") || external.has("TORPLAY_CONFIGURED_NATIVE_PROVIDERS")) {
    throw new SettingsError("Native providers are managed by the host environment.", 409);
  }
  const selected = normalizedNativeProviderIds(ids);
  const configured = normalizedNativeProviderIds(
    options.configuredIds === undefined ? ids : options.configuredIds,
  );
  if (selected.some((id) => !configured.includes(id))) {
    throw new SettingsError("Enabled native providers must also be configured.");
  }
  const enabledValue = selected.join(",");
  const configuredListValue = configured.join(",");
  await atomicWrite(filename, updateSource(source, {
    TORPLAY_NATIVE_PROVIDERS: enabledValue,
    TORPLAY_CONFIGURED_NATIVE_PROVIDERS: configuredListValue,
  }, new Set()), options.platform);
  environment.TORPLAY_NATIVE_PROVIDERS = enabledValue;
  environment.TORPLAY_CONFIGURED_NATIVE_PROVIDERS = configuredListValue;
  return { providerId: "nativeProviders", restartRequired: false };
}

export function updateNativeProviderSettings(ids, options = {}) {
  return serializeSettingsWrite(() => updateNativeProvidersNow(ids, options));
}
...
function normalizedChanges(changes) {
  if (!Array.isArray(changes) || !changes.length) throw new SettingsError("No settings changes were provided.");
  return changes.map((change) => {
    const provider = settingsProvider(change?.providerId);
    if (!provider) throw new SettingsError("Unknown settings provider.");
    return { provider, payload: change.payload || {} };
  });
}

async function prepareChanges(changes, options) {
  const selected = normalizedChanges(changes);
  const environment = options.environment || process.env;
  const filename = options.configPath || settingsConfigPath(environment, options.cwd);
  const source = await readConfigFile(filename);
  const fileValues = parseSettingsEnvironment(source);
  ...
  for (const { provider, payload } of selected) {
    ...
    for (const [id, rawValue] of Object.entries(values)) {
      ...
      removals.delete(field.environment);
    }
    for (const id of remove) {
      const field = known.get(id);
      if (external.has(field.environment)) throw new SettingsError(`${field.label} is managed by the host environment.`, 409);
      delete updates[field.environment];
      removals.add(field.environment);
    }
  }
  const candidateEnvironment = { ...environment, ...updates };
  for (const key of removals) delete candidateEnvironment[key];
  return { candidateEnvironment, environment, filename, removals, selected, source, updates };
}

async function commitPrepared(prepared, options) {
  const { environment, filename, removals, source, updates } = prepared;
...
async function updateProvidersNow(changes, options) {
  const prepared = await prepareChanges(changes, options);
  if (!Object.keys(prepared.updates).length && !prepared.removals.size) {
    throw new SettingsError("No settings changes were provided.");
  }
  await commitPrepared(prepared, options);
  return prepared.selected.map(({ provider }) => ({
    providerId: provider.id,
    restartRequired: provider.id === "omdb",
  }));
}

export async function settingsCandidateEnvironment(changes, options = {}) {
  return (await prepareChanges(changes, options)).candidateEnvironment;
}

export function updateProvidersSettings(changes, options = {}) {
  return serializeSettingsWrite(() => updateProvidersNow(changes, options));
}

export function validateAndUpdateProvidersSettings(changes, validator, options = {}) {
  return serializeSettingsWrite(async () => {
    const prepared = await prepareChanges(changes, options);
    const validation = await validator(prepared.candidateEnvironment);
    if (!validation?.valid) return { committed: false, validation };
    await commitPrepared(prepared, options);
    return { committed: true, validation };
  });
}

export function updateProviderSettings(providerId, payload, options = {}) {
  return updateProvidersSettings([{ providerId, payload }], options).then(([change]) => change);
}
