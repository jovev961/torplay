...
import { effectiveCustomProviders, hasEffectiveTorrentSource } from "./torrent-providers.js";
...
function nativeProviderState(environment, external) {
  ...
  const unknown = selected.find((id) => !NATIVE_PROVIDER_IDS.includes(id));
...
async function updateNativeProvidersNow(ids, options) {
  ...
  if (external.has("TORPLAY_NATIVE_PROVIDERS")) {
  ...
  if (!selected.length && !jackettIsConfigured(environment) && !effectiveCustomProviders(environment).length) {
    throw new SettingsError("Enable at least one native provider or configure Jackett.");
  }
  const value = selected.join(",");
  await atomicWrite(filename, updateSource(source, { TORPLAY_NATIVE_PROVIDERS: value }, new Set()), options.platform);
  environment.TORPLAY_NATIVE_PROVIDERS = value;
...
async function prepareChanges(changes, options) {
  ...
  if (selected.some(({ provider }) => provider.id === "jackett")
    && hasEffectiveTorrentSource(environment) && !hasEffectiveTorrentSource(candidateEnvironment)) {
    throw new SettingsError("Enable another torrent source before removing the last active Jackett connection.");
  }
