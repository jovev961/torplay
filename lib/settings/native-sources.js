...
export async function nativeSourceHealth({ environment = process.env, refresh = false, ...options } = {}) {
  ...
  return Promise.all(saved.map(async (item) => {
    if (!item.enabled) return { provider: item.id, status: "disabled", message: "Source is disabled." };
    if (!active.has(item.id)) return { provider: item.id, status: "disabled", message: "Source is excluded by the provider override." };
