...
import { signalingHealthUrl } from "../watch-together/config.js";
...
async function validateWatchTogether(environment, fetchImpl) {
  if (!configured(environment.WATCH_TOGETHER_SIGNAL_URL)) {
    return result("watch-together", "unconfigured", "Optional service is not configured.");
  }
  let url;
  try { url = signalingHealthUrl(environment.WATCH_TOGETHER_SIGNAL_URL); }
  catch { return result("watch-together", "invalid", "Use a valid HTTPS signaling service base URL."); }
  const response = await safeFetch(url, { headers: { Accept: "application/json" } }, fetchImpl);
  const failure = httpResult("watch-together", response);
  if (failure) return failure;
  const data = await response.json().catch(() => null);
  return data?.status === "ok" && data?.service === "torplay-watch-together"
    ? result("watch-together", "valid", "Signaling connection verified.")
    : result("watch-together", "invalid", "The endpoint is not a TorPlay signaling service.");
}
...
const validators = {
  ...
  subdl: validateSubdl,
  "watch-together": validateWatchTogether,
};
