...
export function publicCustomProviders(environment = process.env, canEdit = false) {
  ...
  return providers.map((provider) => {
    ...
    if (provider.kind === "cardigann") return {
      ...
      definitionId: provider.definition.id, access: provider.definition.type,
      requiresFlareSolverr: provider.definition.settings?.some((item) => item.type === "info_flaresolverr") || false,
...
export async function customProviderHealth({ environment = process.env, refresh = false, ...options } = {}) {
  ...
  return Promise.all(providers.map(async (provider) => {
    if (!active.some((item) => item.id === provider.id)) {
      const result = { provider: provider.id, status: "disabled", message: "Source is disabled or excluded by the provider override." };
