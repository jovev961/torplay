...
function migrateLegacyJackett(providers, environment) {
  ...
  if (!key || key === "replace-me" || !base || providers.some((item) => item.legacyJackett)) return providers;
  if (providers.length >= 20) throw fail("Legacy Jackett settings could not be converted: the source limit has been reached.", 500);
  let origin;
  try {
    origin = new URL(`${base.replace(/\/+$/, "")}/`);
  ...
  if (providers.length + allIds.length > 20) throw fail("Legacy Jackett settings exceed the source limit. No source was converted.", 500);
  const converted = allIds.map((id) => {
    const endpoint = new URL(`api/v2.0/indexers/${id}/results/torznab/api`, origin).toString();
    ...
    return {
      ...
      kind: "torznab", name: id === "all" ? "Jackett (all)" : `Jackett (${id})`, endpoint, apiKey: key,
      enabled: true, legacyJackett: true, searchMediaTypes, genericEnabled: generic.has(id),
  ...
  try {
    ...
    writeFileSync(temporary, JSON.stringify([...providers, ...converted], null, 2) + "\n", { mode: 0o600, flag: "wx" });
    renameSync(temporary, filename);
  } finally { try { unlinkSync(temporary); } catch { /* Rename removed it. */ } }
  return [...providers, ...converted];
}
export function readCustomProviders(environment = process.env) {
  try {
    ...
    return migrateLegacyJackett(data, environment).map((item) => {
...
export function effectiveCustomProviders(environment = process.env, providers = readCustomProviders(environment)) {
  ...
  return providers.filter((item) => item.enabled && (!selected || selected.includes(item.id) || (item.legacyJackett && selected.includes("jackett"))));
}
