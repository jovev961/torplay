...
import { jackettIsConfigured, selectedNativeProviderIds, providerIds } from "../search/native/registry.js";
...
export function hasEffectiveTorrentSource(environment, providers = readCustomProviders(environment)) {
  const override = environment.TORPLAY_SEARCH_PROVIDERS;
  return selectedNativeProviderIds(environment).length > 0
    || (jackettIsConfigured(environment) && (override === undefined || providerIds(override).includes("jackett")))
    || effectiveCustomProviders(environment, providers).length > 0;
}
...
export function changeCustomProvider(action, values, { environment = process.env, ...options } = {}) {
  return serializeSettingsWrite(async () => {
    ...
    if (!hasEffectiveTorrentSource(environment, next)) {
      throw fail("Keep at least one effective torrent source enabled.");
    }
