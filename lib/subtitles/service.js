...
import { sameSubtitlePreferences } from "./preferences.js";
import {
  readCachedSubtitle,
  subtitleCachePath,
  subtitleContentHash,
...
export function startSubtitleDiscovery(session, file, options = {}) {
  ...
  if (existing && sameSubtitlePreferences(existing.preferences, preferences)) return existing;
...
export async function loadSubtitleTrack(session, fileId, trackId, options = {}) {
  ...
  if (track.provider === "torrent") {
    ...
    const cachePath = context
      ? subtitleCachePath(context, track.provider, track.providerId, config)
      : null;
    if (cachePath) discovery.cachePaths.add(cachePath);
    body = context
      ? await readCachedSubtitle(context, track.provider, track.providerId, { config })
      : null;
    if (!body) {
      if (track.provider === "embedded") {
        body = await options.extractEmbedded(discovery.media, track.streamIndex);
      } else {
        body = await downloadProviderSubtitle(track, config, options.fetchImpl);
      }
      if (context) {
        await writeCachedSubtitle(context, track.provider, track.providerId, body, { config });
      }
    }
  }
