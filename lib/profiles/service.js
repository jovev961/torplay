...
function publicProfile(row) {
  ...
  return row ? {
    id: row.id,
    name: row.name,
    avatarId: isProfileAvatarId(row.avatar_id)
      ? row.avatar_id
      : defaultProfileAvatarId(row.id),
    subtitlePreferences,
...
export function createProfile(input, database = getDatabase()) {
  ...
  database.prepare(`
    INSERT INTO profiles (
      id, name, avatar_id, subtitle_default_language, subtitle_languages, created_at, updated_at
    ) VALUES (
      @id, @name, @avatarId, @subtitleDefaultLanguage, @subtitleLanguages, @createdAt, @updatedAt
    )
  `).run(profile);
