...
export function listWatchedTitles(profileId, database = getDatabase()) {
  requireProfile(profileId, database);
  return database.prepare(`
    SELECT media_type AS mediaType, tmdb_id AS tmdbId,
      MAX(last_watched_at) AS lastWatchedAt
    FROM watch_history
    WHERE profile_id = ?
    GROUP BY media_type, tmdb_id
    ORDER BY lastWatchedAt DESC, media_type ASC, tmdb_id ASC
  `).all(profileId);
}
