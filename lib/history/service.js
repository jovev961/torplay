...
export function listContinueWatching(profileId, database = getDatabase()) {
  requireProfile(profileId, database);
  const rows = database.prepare(`
    SELECT * FROM watch_history WHERE profile_id = ?
    ORDER BY last_watched_at DESC, season_number DESC, episode_number DESC
  `).all(profileId);
  return groupedHistory(rows).filter(
    (item) => !item.completed && item.position >= CONTINUE_WATCHING_MIN_SECONDS,
  );
}
